document.addEventListener('DOMContentLoaded', () => {
	...
	async function createNewUrlBox() {
		let objData;
		try {
			objData = await getUrlData(); //data from 1pt.co
			console.log(objData);
			...
			longUrlTextNode.textContent = objData.long;
			longUrlTextNode.classList.add('long-url');

			shortUrlTextNode.textContent = `https://1pt.co/${objData.short}`;
	...
	async function addUrlList() {
		...
		try {
			containerUrlStructure = await createNewUrlBox();
			if (containerUrlStructure) {
				console.log(containerUrlStructure);
				subWrapperShortenUrl.appendChild(containerUrlStructure);
			}
		} catch (error) {
			console.error(error);
		}
